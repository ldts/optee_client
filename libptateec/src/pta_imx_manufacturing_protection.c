...
TEEC_Result pta_imx_mprotect_get_key(char *key, size_t *len)
{
	...
	ret = pta_open_session(&manufacturing_protection_ta_ctx);
	if (ret)
		return ret;
	...
	ret = pta_invoke_cmd(&manufacturing_protection_ta_ctx,
...
{
	return pta_final(&manufacturing_protection_ta_ctx);
}