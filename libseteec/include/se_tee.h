 ...
 *
 * Depending on the actual implementation, this call might only generate new
 * keys once. This could be the case when the board is placed in a secure
 * state and the keys are derived from the HUK instead of read from secure
 * storage.
 *
 * @return SER_CANT_OPEN_SESSION	Error opening the TEE session.
 * @return SER_ERROR_GENERIC		Error unspecified.
